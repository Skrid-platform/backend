...
def process_results(result, query_notes, pitch_gap, duration_gap, sequencing_gap, alpha):
    ...
    sequence_details.sort(key=lambda x: x[2], reverse=True)

    with open("results.txt", "w") as file:  # Open in write mode to clear the file
        rejected_results = len(note_sequences) - len(sequence_details)
        for source, start, sequence_degree, note_details in sequence_details:
