    ...
    for param_idx, param_value in enumerate(param_values):
        for length in range(1, max_length + 1):
            ...
            for seq in range(nb_sequences):
                # print(base_idx + seq * 2, seq, length, param_value)
...
def generate_multiple_random_notes(n, num_notes=15):
    """
    Génère `n` listes de 15 notes à partir de partitions aléatoires.
    ...
    Returns:
        list: Une liste contenant `n` listes de 15 notes.
    """
    ...
    for _ in range(n):
        ...
        # Obtenir les `15+k` premières notes avec `get`
        result = subprocess.run(
